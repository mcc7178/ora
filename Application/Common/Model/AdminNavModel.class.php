...
class AdminNavModel extends BaseModel{
	...
	public function MenuHtml($messages = 0){
		...
		$name = array(0=>'Ordinary_MenuHtml',1=>'Admin_MenuHtml',2=>'Lecturer_MenuHtml',3=>'Student_MenuHtml');

		$MenuHtml = F($name[$groups_id]);
		...
		if(!empty($MenuHtml)){
			...
			foreach($leftMenu as $menu){
				...
				if(strcasecmp($menu['mca'],'admin/index/home') == 0 || strcasecmp($menu['mca'],'admin/test/index') == 0|| strcasecmp($menu['mca'],'admin/audit/auditlist') == 0|| strcasecmp($menu['mca'],'admin/news/page') == 0){
