...
class TaskModel extends CommonModel {
    ...
    public function judgeUser($user_id){
    	$role = M("tissue_group_access")->field("manage_id")->where("user_id=".$user_id)->limit(1)->select();
    	if(!$role){
     ...
     * taskType 任务类型 1 培训项目 2 新建课程、3 新建试卷、4 新建问卷、5 新建话题（工作圈内容发布）、6 发起调研、7 发起考试、8 用户注册、 9 其他（申请积分加分）
     */
    public function detail($param,$user_id){
    	if(!$param || !$user_id){
			return array("code"=>1021, "message"=>'提交失败，未获取到参数或用户数据');
		}
     ...
     */
    public function operate($param,$user_id){
    	if(!$param || !$user_id){
			return array("code"=>1021, "message"=>'提交失败，未获取到参数或用户数据');
		}
		...
		//9申请积分加分 integration_apply
			...
			if($param["taskStatus"] == 1){
				...
				$inteData["time"] = time();
				$inteData["user_id"] = $detail[0]["user_id"];
