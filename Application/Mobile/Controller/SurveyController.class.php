...
class SurveyController extends CommonController{
	...
	public function getList(){
		...
		//处理试卷过期
		D("survey")->overdue($userId);

		//添加所属组织参加试卷状态
		D("survey")->researchAdd($userId);

		$return = D("survey")->getList($param,$userId);
	...
	public function finish(){
		...
		$post["project_id"] += 0;
		$post["research_id"] += 0;
		$post['answer'] = I("post.answer",'','strip_tags,stripslashes,trim');
	...
	public function seeDetail(){
		...
		$this->assign($data);
        $this->assign('datas',$data['data']['option']);
		...
		if($data["code"] == 1000){
			$data = D("survey")->seeDetail($post, $user_id);
		}
