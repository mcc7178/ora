...
class MineController extends CommonController {
   ...
   */
    public function myCourse(){
        ...
        $info = D('Mine')->myCourse($typeId,$page,$pageNum = 10,$userId);
        if($info['list']){
   ...
   public function news(){
     ...
     * 资讯列表-bak
     */
    public function newsList_bak(){
        //判断用户是否存在,获取用户id,判断提交方式是否合法
        $userId = $this->verifyUserDataGet();
        if(strtolower(C('DB_TYPE')) == 'oracle'){
            $field = "id,title,type,content,img,auth_user_id,user_id,tissue_id,template,to_char(create_time,'YYYY-MM-DD HH24:MI:SS') as create_time";
        }else{
            $field = "id,title,type,content,create_time,img,auth_user_id,user_id,tissue_id,template";
        }
        $info = M('News')->field($field)->order('id DESC')->select();
        


        foreach($info as $key => $val){
            $content = htmlspecialchars_decode($val['content']);
            $info[$key]['content'] = str_replace('&nbsp;','',strip_tags($content));;
        }
        if($info){
            $this->success(1000,'获取成功',$info);
        }else{
   ...
   public function getTissueId($user_id){
    ...
    public function newsList(){
        ...
        //对我共享
        $ret = M('resource_sharing')->where(array('tissue_id'=>$tissue_id,'type'=>8))->select();
        $source_id_arr = array();
        
        foreach($ret as $k=>$v){
            $source_id_arr[] = $v['source_id'];
        }
        $source_ids = implode(',',$source_id_arr);
		...
		if(strtolower(C('DB_TYPE')) == 'oracle'){
			...
			$news = $model->where($where)->page($page, $size)->order('id desc')->select();
		}
        ...
        foreach($news as $key => $val){
            $content = htmlspecialchars_decode($val['content']);
            $news[$key]['content'] = str_replace('&nbsp;','',strip_tags($content));;
        }
        if($news){
            $this->success(1000,'获取成功',$news);
        }else{
            $this->error(1030,'无数据返回');
        }
    ...
    public function systemNews(){
        ...
        $info = D('Mine')->getSystemNews($userId,$page,$tabType);
        if($info['code'] == 1000){
            $this->success($info['code'],$info['message'],$info['data']);
        }else{
            $this->error($info['code'],$info['message']);
    ...
    public function learningTarget(){
        ...
        if($typeId == 1){
            ...
            $info = D('Mine')->learningTarget(3,$page,$userId);
            $info['token'] = $token;
            $info['secret_key'] = $secret_key;
            $this->assign('items', $info);
    ...
    public function  credit(){
        ...
        $info = D('Mine')->credit($userId,$start_page,$pageNum);
        //获取中保协学习数据
        $set = M("iac_set")->find();
        if($set['status'] == 1) {
            $Iac = D('Admin/Iac');
            $iacStudy = $Iac->getIacStudy($userId);
            $info["total_score"] = $info["total_score"] + $iacStudy["study_credit"];
            //$data["hours"] = $data["hours"] + round($iacStudy["study_len"] / 3600, 2);
            //$data["finishedCourseNum"] = $data["finishedCourseNum"] + $iacStudy["study_num"];
        }
        if(empty($info['data']) && empty($info['total_score'])){
            $this->error(1030,'无数据返回');
        }else{
            $this->success(1000,'获取数据成功',$info);
        }
    ...
    public function recordData(){
        ...
        $info = D('Mine')->recordData($user_id,$course_id,$chapter_id,$project_id,$name,$path,$type,$style);
        if($info == 1){
