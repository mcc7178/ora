...
class AdminTissueModel extends BaseModel{
	...
	public function index($total_page = 10){
		...
		$jobs_id = I("get.jobs_id/d");

		$tag_id = I("get.tag_id/d");
		...
		//指定用户只分中心
		if(($typeid == 2 OR $typeid == 3) && $group_data['pid'] != 0){

			$rule_items = $this->leveldata($group_data['tissue_id']);
		...
		$parentName = str_replace($qian,$hou,$parentName);
	...
	public function addUser(){
		...
		$pwdAuth = A('Info')->pwdAuth($post['password']);

		if($pwdAuth['code'] == 1021){
			return 410;
		}
		...
		if(!empty($data['username']) && !empty($data['password']) && !empty($data["email"])){
			...
			try {
				...
				if(strtolower(C('DB_TYPE')) == 'oracle'){
					...
					$data['birthday'] = array('exp',"to_date('".$data['birthday']."','yy-mm-dd')");
				}
				...
				if($increment_id){
					...
					if(strtolower(C('DB_TYPE')) == 'oracle'){
						M('auth_group_access')->add(array("user_id"=>$data['id'],"group_id"=>3));
						M('tissue_group_access')->add(array("user_id"=>$data['id'],"tissue_id"=>$tissue_id,"job_id"=>$jobs_id,"manage_id"=>0));
					}else{
					...
					foreach($tag_list as $tag){
						...
						if(strtolower(C('DB_TYPE')) == 'oracle'){
							$tag_data['id'] = getNextId('users_tag_relation');
						}

						M('users_tag_relation')->add($tag_data);
					}

					$user_info = M("Users")->where("id=".$increment_id)->find();

					write_pwd_history($user_info['id'],$user_info['password']);
	...
	public function moveUser(){
		...
		if($typeid == 1){
			...
			$tissue_Id =  I('post.tissue_Id','','stripslashes,trim');

			$tissue_list = json_decode($tissue_Id,true);

			//删除当前所属组织
			M("tissue_plan")->where("user_id=".$user_id)->delete();

			//重新赋予新组织
			foreach($tissue_list as $id){

				$where["plan_id"]  = array("eq",$id);

				$data = array("user_id"=>$user_id,"plan_id"=>$id);

				M('tissue_plan')->add($data);
			...
			$ids =  I('post.ids','','stripslashes,trim');
	...
	public function editUser(){
		...
		if(strtolower(C('DB_TYPE')) == 'oracle'){
			$user = M("Users a")
				->field("a.id,a.username,a.password,a.avatar,a.email,a.phone,a.status,a.register_time,a.last_login_ip,a.last_login_time,a.job_number,a.province,a.city,a.personalized_signature,a.qrcode,a.audit_time,a.orderno,a.objection,a.sequence,to_char(a.birthday,'YYYY-MM-DD') birthday,a.gender,a.age,to_char(a.group_time,'YYYY-MM-DD') group_time,to_char(a.center_time,'YYYY-MM-DD') center_time,a.area,a.room,a.rank,a.education,a.mobilephone,a.tel,a.ip_phone,b.tissue_id as tid,b.job_id")
	...
	public function UpdateUser(){
		...
		if(!empty($is_phone)){
			...
			try {
				...
				if(!empty($post['password'])){
					write_pwd_history($post['id'],md5($post['password']));
				}
	...
	public function delTissue(){
		...
		$tissue_id_list = D('IsolationData')->ruleData($tissue_id);

		if(!empty($tissue_id_list)){

			$id_list = array_merge($tissue_id_list,array($tissue_id));
			
			$where['a.tissue_id'] = array("in",$id_list);
		}else{
	...
	public function addbatchmarking(){
		...
		$list = I('post.list','','stripslashes,trim');

		$tag_id = I('post.tag_id','','stripslashes,trim');
		...
		try {
			...
			foreach($items as $val){
				...
				foreach($tag_list as $tag){
					...
					if(strtolower(C('DB_TYPE')) == 'oracle'){
						$data['id'] = getNextId('users_tag_relation');
					}
	...
	public function rightajax(){
		...
		if($typeid == 1){
			...
			$user_id = I('post.user_id','','stripslashes,trim');

			$user_list = json_decode($user_id);
			...
			$rightajax= F('rightajax');


			//获取左则选项所有用户
			$tissue_id = json_decode($tissue_id);

			foreach($tissue_id as $k=>$id){

				if($id != $rightajax['tissue_id']){
					$where['a.tissue_id'] = array("eq",$id);
					$where['b.status'] = array("eq",1);

					//获取用户上级组织名称
					$left_user_list[$k] = M("tissue_group_access a")
						->join("LEFT JOIN __USERS__ b ON a.user_id = b.id")
			...
			$l = 0;
			foreach($left_user_list as $k=>$user_list){
				foreach($user_list as $i=>$list){
					$rows[$l]['id'] = $list['id'];
					$rows[$l]['username'] = $list['username'];
					$l++;
				}
			}

			//获取右侧勾选用户
			if(!empty($rightajax['user_list'])){

				$map['id'] = array("in",$rightajax['user_list']);
				$map['status'] = array("eq",1);
				$right_user_list = M("users")->field("id,username")->where($map)->select();

				foreach($right_user_list as $k=>$user){
					$right_rows[$k]['id'] = $user['id'];
					$right_rows[$k]['username'] = $user['username'];
				}

				if(empty($rows)){

					$user_data = $right_rows;

				}else{

					$user_data = array_merge($rows,$right_rows);
