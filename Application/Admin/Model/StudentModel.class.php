...
class StudentModel extends BaseModel{
    ...
    public function getStudentInfo(){
		...
		//已完成的课程数量
		$finishedCourseNum = M('center_study')->where(array('user_id'=>$user_id,'hours'=>array('NEQ',0)))->count();
		
		//已完成的课程学时
        $hours = M('center_study')->where(array('user_id'=>$user_id))->sum('hours');
		$hours = $hours ? $hours : 0;
		...
		$credit = $up_credit + $downCredit;
        $thisHours = $up_time + $down_time;
        $hours = round($thisHours / 60, 2);
        $down_time = 0;
        
        //课程信息
		$where1['a.user_id'] = $user_id;
        $course2 = M('course_chapter a')
    		->join('left join __COURSE__ b on a.course_id=b.id')
			->field("a.user_id,a.course_id,a.project_id")
    		->where($where1)
//  		->group('a.user_id,a.course_id,a.project_id')
    		->select();
    	
    	foreach($course2 as $k=>$v){
    		unset($course2[$k]['numrow']);
        }

	    foreach ($course2 as $v){
    	...
    	foreach($temp as $k=>$v){
    		$courseInfo[] = M('course')->where(array('id'=>$v[1]))->find();
			$per = $this->getCoursePer($v[0],$v[1],$v[2]);
            $courseInfo[$k]['per'] = $per;
            $courseInfo[$k]['course_id'] = $v[1];
            $courseInfo[$k]['project_id'] = $v[2];
    	}
		
    	$courseSort = $this->courseSort($courseInfo);
        $courseRes = array_slice($courseSort,0,8); //只取前面8个做展示*/
        ...
        if(!empty($plan_id)){
            ...
            $conditions['auth_user_id'] = array("in",array(0));
        }

       // dump($specifiedUser);exit;
        ...
        //$conditions['_string'] = ' (a.tissue_id='.$tissue_id.' AND a.template=1 ) OR ( a.template=2 ) ';
        ...
        if(strtolower(C('DB_TYPE')) == 'oracle'){
            ...
            $news = $model->where($conditions)->limit(10)->order('b.id desc')->select();
        }
        
       // $news = $news ? $news : array();
        $news = $news ? $news : array();
       // $newsAll = array_merge($news,$news2);
        //$newsAll = array_slice($newsAll,0,10);
    ...
    public function getSchedule(){
        ...
        $course = $course ? $course : array();
        $test['list'] = $test['list'] ? $test['list'] : array();
        $survey['list'] = $survey['list'] ? $survey['list'] : array();
