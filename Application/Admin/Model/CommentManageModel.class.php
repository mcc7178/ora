...
class CommentManageModel extends BaseModel{
	...
	public function index($param){
		...
		$planId = getPlanId();
		//获取课程分类(三级分类，剩下的作为平级)
		$courseCat = M("course_category")->where("pid=0 and plan_id=".$planId)->select();
		...
		$count = M("colligate_comment a")->field("count(DISTINCT(a.course_id)) as t_num")
			->join("JOIN __COURSE__ b ON a.course_id=b.id")
			->join("LEFT JOIN __COURSE_CATEGORY__ c ON b.course_cat_id = c.id")
			->where($where)->select();
		
		//输出分页
		$pageNav = $this->pageClass($count[0]["t_num"], $param["pageLen"]);
