...
class MyCourseModel extends BaseModel{
	...
	public function index($param){
		...
		$map['pid'] = array("eq",0);
		$courseCat = M("course_category")->where($map)->select();
		foreach ($courseCat as $k1=>$v1){
			$subCat1 = M("course_category")->where("pid=".$v1["id"])->select();
			foreach ($subCat1 as $k2=>$v2){
				$subCids = self::getCourseChild($v2["id"], $v2["id"].",");
		...
		if($param["type"] == 1){
			...
			if($param["timeType"] == 1){
				//进行中
				$timeLimit = date("Y-m-d 00:00:00");
				$where["_string"] = " c.end_time >= to_date('$timeLimit','yyyy-mm-dd hh24:mi:ss')";
			}elseif($param["timeType"] == 2){
				//已结束
				$timeLimit = date("Y-m-d 00:00:00");
				$where["_string"] = " c.end_time < to_date('$timeLimit','yyyy-mm-dd hh24:mi:ss')";
			}
			...
			foreach ($results as $key=>$value){
				...
				$course_detail = M("course_detail")->where("id=".$value["course_id"])->find();
				$results[$key]["course_description"] = $course_detail["course_intro"];
				
				
				//计算进度//0在线，1面授
				$startStatus = strtotime($value['start_time']) - time();//大于0表示待考试
				$endStatus = strtotime($value['end_time']) - time();//小于0表示逾期（已结束）
				
				$results[$key]["jindu"] = "学习中";
				if($startStatus > 0){
					$results[$key]["jindu"] = "未开始";
				}
				if($endStatus < 0){
					$results[$key]["jindu"] = "已结束";
				}
				
				if($value["course_way"] == 0){
					if(!$value["chapter"]){
						continue;
					}
					//在线课程
					...
					//查询已学习的章节
					$chapWheres['project_id'] = $value["project_id"];
					$chapWheres['course_id'] = $value['id'];
					$chapWheres['user_id'] = $user_id;
					$chapWheres['status'] = 3;
					$isStudy = M("course_chapter")->field("count(id) as t_num")->where($chapWheres)->select();
					
					if($chapter == $isStudy[0]["t_num"]){
						$results[$key]["jindu"] = "已学完";
					}
				}else{
					//面授课程
					//面授课程数据计算
			...
			foreach ($results as $key=>$value){
				...
				$course_detail = M("course_detail")->where("id=".$value["id"])->find();
				$results[$key]["course_description"] = $course_detail["course_intro"];
				...
				//查询已学习的章节
				$chapWheres['project_id'] = 0;
				$chapWheres['course_id'] = $value['id'];
				$chapWheres['user_id'] = $user_id;
				$chapWheres['status'] = 3;
				$isStudy = M("course_chapter")->field("count(id) as t_num")->where($chapWheres)->select();
				
				unset($chapWheres['status']);//不看状态，是否有学习数据
				$unStudy = M("course_chapter")->field("count(id) as t_num")->where($chapWheres)->select();
				if($unStudy){
					$results[$key]["jindu"] = "学习中";
				}
			...
			$results = M("course a")
			->join("LEFT JOIN __COURSE_CATEGORY__ b ON a.course_cat_id = b.id")
			->join("JOIN __COURSE_CARE__ c ON a.id = c.course_id")
			->field("a.*,b.cat_name")->where($where)->limit($start, $param["pageLen"])->select();
			foreach ($results as $key=>$value){
				$results[$key]["project_id"] = "true";
			...
			$count = M("course a")
				->join("LEFT JOIN __COURSE_CARE__ c ON a.id = c.course_id")
				->field("a.id")->where($where)->select();
	...
	public function base($param){
		...
		if($course){
			...
			$course['chapter'] = str_replace("&quot;", '"', $course['chapter']);//
			...
			foreach ($course['chapter'] as $key=>$value){
				...
				if(strstr($fileType, ".mp4") || strstr($fileType, ".flv") || strstr($fileType, ".ogv") || strstr($fileType, ".webm")){
					...
					$fileType = "pdf";
				}elseif(strstr($value["src"], "scorm/course")){//mdzz
					...
					$fileType = "kejian";
				}elseif(getimagesize($value["src"])){
					$fileType = "image";
				}elseif(strstr($fileType, ".html")){
					//office转换预览
					$course['chapter'][$key]["show_doc"] = "true";
					$fileType = "doc";
				}else{
				...
				if($showDoc["view_file_path"] && in_array($fileType, $officeType)){
					$course['chapter'][$key]["show_doc"] = "true";
					$course['chapter'][$key]["src"] = "http://".$_SERVER['SERVER_NAME']."/admin/office/showDoc?course_id=".$param["course_id"]."&chapter_id=".$key;
				} */
				...
				$cwhere["project_id"] = (int)$param["project_id"];
			...
			//是否收藏
			$whereCare["user_id"] = $user_id;
			$whereCare["course_id"] = $param["course_id"];
			$course_care = M("course_care")->where($whereCare)->find();
			$is_care = 2;
			if($course_care){
				$is_care = $course_care["care_status"];
			}
			$course["is_care"] = $is_care;
			...
			$course['course_score'] = ceil($comment[0]["course_score"]);
			...
			$course['lecturer_score'] = ceil($comment[0]["lecturer_score"]);
	...
	public function getBaseInfo($param){
		...
		if($param["project_id"] == "true"){
			...
			$course = M("project_course")->field("to_char(start_time,'YYYY-MM-DD HH24:MI:SS') as start_time,to_char(end_time,'YYYY-MM-DD HH24:MI:SS') as end_time")
				->where("project_id=".$param["project_id"]." AND course_id=".$param["course_id"])->find();
	...
	public function getComment($param){
		...
		foreach ($comList as $key=>$value){
			//echo "<br>aaaa01--".microtime();
			$comList[$key]["comment_content"] = htmlspecialchars_decode($comList[$key]["comment_content"]);//实体转换为html
			...
			if($pids){
				...
				if($sonCon){
					...
					foreach ($sonCon as $sk=>$sv){
						$sonCon[$sk]["comment_content"] = htmlspecialchars_decode($sonCon[$sk]["comment_content"]);//实体转换为html
	...
	public function makePraise($param){
		...
		if($has){
			...
			$resp = M("course_praise")->where("user_id=".$user_id." AND id=".$param["comment_id"])->save($data);
	...
	public function delComment($param){
		$user_id = $_SESSION["user"]["id"];
		$resp = M("colligate_comment")->where("id=".$param["comment_id"])->delete();
		if($resp){
			//删除关联的回复评论 
	...
	public function getNote($param){
		...
		if(!$param["pageLen"]) $param["pageLen"] = 15;
		$start = ($param["page"] - 1) * $param["pageLen"];
		if($param["project_id"] == "true" || $param["project_id"] == "false") $param["project_id"] = 0;
		if(strtolower(C('DB_TYPE')) == 'oracle'){
			$table1 = "SELECT id,user_id,note_content,to_char(time, 'YYYY-MM-DD HH24:MI:SS') as time FROM __COURSE_NOTE__ WHERE course_id=".$param["course_id"]." AND project_id=".$param["project_id"]." AND ((is_public=1 AND user_id!=".$user_id.") OR (user_id=".$user_id."))";
		}else{
			$table1 = 'SELECT id,user_id,note_content,time FROM __COURSE_NOTE__ WHERE course_id='.$param["course_id"].' AND project_id='.$param["project_id"].' AND ((is_public=1 AND user_id!='.$user_id.') OR (user_id='.$user_id.'))';
		}
		$sql = $table1.' ORDER BY time DESC';
		$data = M()->query($sql);
	...
	public function delNote($param){
		M("course_note")->where("id=".$param["note_id"])->delete();
		return array("code"=>1000, "message"=>"成功");
	}
	...
	public function giveScore($param){
		...
		if($has){
			...
			$return = M("course_score")->where("user_id=".$user_id." AND course_id=".$param["course_id"])->save($data);
			if($param["type"] == 1){
				//课程评价
				$comment = M("course_score")->field("avg(course_score) as course_score")->where("course_id=".$param["course_id"]." AND course_score>0")->select();
				if(!$comment[0]["course_score"]) $comment[0]["course_score"] = 0;
				$avgScore = ceil($comment[0]["course_score"]);
				M("course")->where("id=".$param["course_id"])->save(array("score"=>$avgScore));
			}else{
				//讲师评价
				...
				M("lecturer")->where("id=".$has[0]["lecturer_id"])->save(array("score"=>$avgScore));
			}
			...
			if(strtolower(C('DB_TYPE')) == 'oracle'){
				$data["score_time"] = array('exp',"to_date('".date('Y-m-d H:i:s')."','yy-mm-dd hh24:mi:ss')");
				$data['id'] = getNextId('course_score');
			}
			...
			if($return){
				if($param["type"] == 1){
					...
					if(!$comment[0]["lecturer_score"]) $comment[0]["lecturer_score"] = 0;
					$avgScore = ceil($comment[0]["lecturer_score"]);
					M("lecturer")->where("id=".$lecturer[0]["lecturer_id"])->save(array("score"=>$avgScore));
				}
	...
	public function addCourse($param){
		...
		if(strtolower(C('DB_TYPE')) == 'oracle'){
			$data['id'] = getNextId('course_record');
		}
	...
	public function publicCourse($param){
		...
		$wheres['b.user_id'] = array("eq",$_SESSION['user']['id']);
		...
		if($plan_id > 0){
			$where101['auth_user_id'] = array("in",$userid_all);
		}else{
			$where101['auth_user_id'] = 0;
		}
		$tissue_course = M("course")->where($where101)->field("id")->select();
