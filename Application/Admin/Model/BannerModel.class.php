   ...
   protected $tableName= 'company_banner';
     ...
     */
     public function bannerList()
     {
         ...
         $lists = M('Company_banner')->order('banner_img_site asc')->where($where)->select();
     ...
     public function add()
     {
        ...
        //判断图片位置是否给占用
         $where = array(
             ...
             'auth_user_id'=>intval(session('user.id')),//20171110新增
             'plan_id' => $plan_id
         );
        if($id){
        ...
        $ajaxdata = array(
            ...
            'auth_user_id' => intval(session('user.id')),//20171108新增auth_user_id字段用作数据隔离
            'plan_id' => $plan_id
        ...
        if(empty($id)){
           //判断图片总数最大为5张
	        $count = M('company_banner')->where(array('plan_id' => $plan_id))->count();
	        if($count >= 5){
     ...
     */
     ...
     {
         $plan_id = getPlanId();//获取方案id
       $banner_img_site = M('company_banner')->where(array('id'=>$id,'plan_id' => $plan_id))->getField('banner_img_site');
     ...
     */
     ...
     {
         $plan_id = getPlanId();//获取方案id
       $lists = M('Company_banner')->where(array('plan_id' => $plan_id))->order('banner_img_site asc')->select();
       foreach($lists as $k=>$v){
     ...
     */
     public function speedSet()
     {
         ...
         $res = M('Company_banner')->where(array('plan_id' => $plan_id))->save($data);
         if(!$res){
            $this->error = '设置失败';
            return false;
        } 
     ...
     */
     public function orderNumber(){
       ...
       $dataone = M('company_banner')->where(array('id'=>$id,'plan_id' => $plan_id))->find();
       if($order == 1){
     ...
     */
     public function getItem($type)
     {
       ...
       //获取搜索条件
         ...
         $map1['auth_user_id'] = array("in",$specifiedUser);
         $map2 = array('course_name'=>array('like','%'.$condition.'%'));
         $map2['auth_user_id'] = array("in",$specifiedUser);
         $map3 = array('research_name'=>array('like','%'.$condition.'%'));
         $map3['auth_user_id'] = array("in",$specifiedUser);
