      ...
      //  }
       ...
       $maps['a.is_delete'] = array('eq',0);
	   	...
	   	if(strtolower(C('DB_TYPE')) == 'oracle'){
			$field = "a.id,a.type_id,a.title,c.type_name,a.status,b.username as fromname,to_char(a.newstime,'YYYY-MM-DD HH24:MI:SS') as newstime";
		}else{
			$field = "a.id,a.type_id,a.title,c.type_name,a.status,a.newstime,b.username as fromname";
		}
       $list[] =  $this->alias('a')
                       ->field($field)
    ...
    public function topicaudit(){ 
       if (IS_AJAX) { 
         ...
         if(strtolower(C('DB_TYPE')) == 'oracle'){
           ...
           'status'=>$status,
           'add_time'=>array('exp',"to_date('".date('Y-m-d H:i:s')."','yy-mm-dd hh24:mi:ss')")
           );
          }
