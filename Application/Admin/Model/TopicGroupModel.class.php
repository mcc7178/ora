...
class TopicGroupModel extends BaseModel{
    ...
    */
    public function applyToJoin(){ 
       if(IS_AJAX){
         ...
         if(strtolower(C('DB_TYPE')) == 'oracle'){
			$pdata['id'] = getNextId('topic_personnel');
	     } 
    ...
    //    dump($fdata);die;
       ...
       $orderno_data = D('Trigger')->orderNumber(10);
    ...
    //    echo $orderno;   die;
       ...
       if(strtolower(C('DB_TYPE')) == 'oracle'){
			$gdata['id'] = getNextId('topic_group');
			$gdata['publish_time'] = array('exp',"to_date('".date('Y-m-d H:i:s')."','yy-mm-dd hh24:mi:ss')");
		}

       if($orderno_data['status'] == 0) $gdata['status'] = 1;
     ...
     */
     public function getgroupTpoicList($group_id)
     {
       ...
       $list = M('group_topic')->alias('a')
                ...
                ->field("a.id,a.topic_group_id,a.user_id,to_char(a.publish_time,'YYYY-MM-DD HH24:MI:SS') as publish_time,a.praise,a.name,a.describe,a.orderno,a.status,a.topic_covers,b.username" )
        ...
        $list = M('group_topic')->alias('a')
                ->join('left join __USERS__ b on a.user_id=b.id')
                ->field("a.id,a.topic_group_id,a.user_id,a.publish_time,a.praise,a.name,a.describe,a.orderno,a.status,a.topic_covers,b.username" )
                ->where(array('topic_group_id'=>$group_id))
                ->page($p.','.$size)
                ->order('a.publish_time desc')
                ->select();   
    ...
    //    $orderno = D('Trigger')->orderNumber(10);
       $content = I('post.content','','stripslashes,trim');
       ...
       if(strtolower(C('DB_TYPE')) == 'oracle'){
			$gdata['id'] = getNextId('group_topic');
            $gdata['publish_time'] = array('exp',"to_date('".date('Y-m-d H:i:s')."','yy-mm-dd hh24:mi:ss')");
		}
      
    
       $gdata['describe'] = preg_replace("/&lt;[\/]?[a-z-A-Z]+&gt;/is","",$gdata['describe']);
        //   DUMP($gdata);die;
    ...
    */
     public function add()
	 {
       ...
       if(I('post.type') == 'public'){ //发布
           $orderno = D('Trigger')->orderNumber(5); //此处废弃
           $data = array(
           'orderno'=>$orderno,
           'topic_id'=>$topic_id,
           'user_id'=>$_SESSION['user']['id'],
    ...
    */
     public function getTheTopic($topic_id)
	 {
        ...
        if(strtolower(C('DB_TYPE')) == 'oracle'){
         $data = M('group_topic')->alias('a')
                 ->join('left join __USERS__ b on a.user_id=b.id')
                 ->field("a.id,a.topic_group_id,a.user_id,a.name,a.describe,a.status,a.topic_covers,to_char(a.publish_time,'YYYY-MM-DD HH24:MI:SS') as publish_time,b.username,b.avatar")
                 ->where(array('a.id'=>$topic_id))
                 ->find();
        }
        return $data; 
