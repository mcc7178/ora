	...
	*/
	...
	public function my($table_search,$id){
		...
		//小组（部落）的所有成员操作展示
         ...
         $data = M('topic_group a')->field("a.name,b.id,b.topic_group_id,b.user_id,b.apply_reason,b.status,b.group_leader,to_char(b.add_time,'YYYY-MM-DD HH24:MI:SS') as add_time")
		  ...
		  $data = M('topic_group a')->field("a.name,b.id,b.topic_group_id,b.user_id,b.apply_reason,b.status,b.group_leader,b.add_time")
									->join('join __TOPIC_PERSONNEL__ b on a.id=b.topic_group_id')
									->join('join __USERS__ c on b.user_id=c.id')
									->where($condition)
									->where(array('a.id'=>$id,'a.status'=>1,'b.status'=>1))
		 ...
		 $count = M('topic_group a')->join('join __TOPIC_PERSONNEL__ b on a.id=b.topic_group_id')
									->join('join __USERS__ c on b.user_id=c.id')
									->where($condition)
									->where(array('a.id'=>$id,'a.status'=>1,'b.status'=>1))
									->count();
		...
		//小组（部落）的所有话题展示
		if(strtolower(C('DB_TYPE')) == 'oracle'){
        $alltopic = M('group_topic')->field("id,topic_group_id,user_id,to_char(publish_time,'YYYY-MM-DD HH24:MI:SS') as publish_time,praise,name,describe,orderno,status,topic_covers")
		                            ->where(array('topic_group_id'=>$id))->select();
		}else{
		$alltopic = M('group_topic')->field("id,topic_group_id,user_id,publish_time,praise,name,describe,orderno,status,topic_covers")
		                            ->where(array('topic_group_id'=>$id))->select();	
		}
	...
	public function aboutMe($type){
		...
		if($type == 1){
			...
			if(empty($myid)){
			  ...
			  if(strtolower(C('DB_TYPE')) == 'oracle'){
              $data = M('topic_interaction')->field("id,topic_id,user_id,content,images,to_char(publish_time,'YYYY-MM-DD HH24:MI:SS') as publish_time,status,pid")
			                                ->where(array('pid'=>array('in',$myid)))->order('id desc')->page($p.','.$size)->select();
			  }else{
			  $data = M('topic_interaction')->field("id,topic_id,user_id,content,images,publish_time,status,pid")
			                                ->where(array('pid'=>array('in',$myid)))->order('id desc')->page($p.','.$size)->select();  
			  }
			...
			if(strtolower(C('DB_TYPE')) == 'oracle'){
				$data = M('group_topic a')->field("a.name,b.topic_id,b.user_id,b.content,to_char(b.publish_time,'YYYY-MM-DD HH24:MI:SS') as publish_time")
			 	...
			 	$data = M('group_topic a')->field("a.name,b.topic_id,b.user_id,b.content,publish_time")
										->join('join __TOPIC_INTERACTION__ b on a.id=b.topic_id')
										->order('b.publish_time desc')
										->where(array('a.user_id'=>$user_id,'a.status'=>1))
										->page($p.','.$size)
