...
class IndexAdminModel extends BaseModel{
	...
	public function actived(){
		...
		$sql = "SELECT tmp_table.*,to_char(tmp_table.end_time,'YYYY-MM-DD HH24:MI:SS') as end_time,admin_pro.project_name,admin_pro.population,admin_pro.is_public FROM (".$sql1." UNION ".$sql2." UNION ".$sql3.") tmp_table 
	...
	public function news(){
        ...
        if(strtolower(C('DB_TYPE')) == 'oracle'){
			...
			$news = M("news")->where($conditions)->field("id,title,type,create_time")->order("create_time DESC")->limit(5)->select();
		}
		if($news){
		for ($i=0; $i<5; $i++){
			if(!$news[$i]){
