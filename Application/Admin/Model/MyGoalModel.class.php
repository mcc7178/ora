...
//	初始化
    ...
    public function getData(){
		...
		$lecturer = M("lecturer")->where("user_id=".$user_id." AND is_delete='0'")->select();
		if($lecturer){
			//只获取面授课程，只有此类才有授课时长
			...
			foreach ($publicCourse as $key=>$value){
				...
				if(strtolower(C('DB_TYPE')) == 'oracle'){
					...
					$project_course = M("project_course a")
						->field("a.project_id,to_char(a.start_time,'YYYY-MM-DD HH24:MI:SS') as start_time,to_char(a.end_time,'YYYY-MM-DD HH24:MI:SS') as end_time")
						->join("JOIN __ADMIN_PROJECT__ b ON a.project_id=b.id")
						->where("a.course_id=".$value["id"]." and (b.type=4 or b.type=0) and a.start_time>to_date('".date("Y-01-01 00:00:00")."','yyyy-mm-dd hh24:mi:ss')")
						->select();
	...
	public function course_data($total_page){
		...
		if(strtolower(C('DB_TYPE')) == 'oracle'){
			...
			if(!empty($keywords)) {
				$where .= " and a.course_name like '%$keywords%'";
			}

			if($arrangement_id >= 1){
				$where .= " and a.arrangement_id = $arrangement_id";
			}
			...
			if($classid == 1){
				...
				$results = M("course a")->join("LEFT JOIN __COURSE_CATEGORY__ b ON a.course_cat_id = b.id")->field("a.id,a.course_name,a.course_time,a.arrangement_id,a.create_time,b.cat_name")->order("a.create_time desc")->where($where)->select();

			}else{

				$results = M("course a")->join("LEFT JOIN __COURSE_CATEGORY__ b ON a.course_cat_id = b.id")->field("a.id,a.course_name,a.course_time,a.arrangement_id,a.create_time,b.cat_name")->order("a.create_time desc")->where($where)->page($start_page,$total_page)->select();
			...
			if($classid == 1){
				...
				$results = M("course a")->join("LEFT JOIN __COURSE_CATEGORY__ as b ON a.course_cat_id = b.id")->field("a.id,a.course_name,a.course_time,a.arrangement_id,a.create_time,b.cat_name")->order("a.create_time desc")->where($where)->select();

			}else{

				$results = M("course a")->join("LEFT JOIN __COURSE_CATEGORY__ as b ON a.course_cat_id = b.id")->field("a.id,a.course_name,a.course_time,a.arrangement_id,a.create_time,b.cat_name")->order("a.create_time desc")->where($where)->page($start_page,$total_page)->select();
	...
	public function teaching_data($total_page){
		...
		if($classid == 1){
			...
			if(strtolower(C('DB_TYPE')) == 'oracle'){
				...
				$map['b.user_id'] = array('exp','is not null');
		...
		foreach($results as $k=>$list){
				...
				if($course_score > 0){
					...
					$satisfaction = round(( $course_score / (5 * $people_number)) * 100,2);
				}else{
					$satisfaction = 0;
	...
	public function all_data(){
		...
		$so['c.user_id'] = array('exp','is not null');
		$teaching_num = M("project_course a")
					->join("LEFT JOIN __COURSE__ b ON a.course_id = b.id LEFT JOIN __ADMIN_PROJECT__ c ON a.project_id = c.id")
					->where($so)
					->count();
