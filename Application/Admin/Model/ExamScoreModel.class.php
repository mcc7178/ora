...
class ExamScoreModel extends BaseModel
{
	...
	*/
	...
	/**
	 * 获取考试成绩
	 * $map 参数信息:project_id 项目ID  exam_id 试卷ID type:导出分数类型 1-全部 0-最高
	 * $type 导出试卷--类型  1:所有   0:最高分
	 */
    public function getAllExaminationResults2($total_page = 10,$map){
        $start_page = I("get.p",1,'int');
        $where = array();
        $where["a.project_id"]=$map["project_id"];
        $where["a.sign_up"]=1;	//已报名
        // $where["a.exam_id"]=$map["exam_id"];
...
//              ->page($start_page,$total_page)
        ...
        foreach($info as $k=>$v){
        	//本次试题是第几次考试
			$max = D('MyExam')->getCounter(array('project_id'=>$v['project_id'],'examination_id'=>$map['exam_id']));
			
            $info[$k]['attendance'] = $this->getAttStatus($v['project_id'],$v['user_id'],$map['exam_id']);
//          $info[$k]['total_score'] = $this->getTestScore($v['project_id'],$v['user_id'],$map['exam_id'],$max);
        }
		...
		foreach($info as $k=>$v){
			$map['exam_id'] = $map['exam_id'] ? $map['exam_id'] : 0;
			$freq = M('project_examination')->where(array('project_id'=>$map["project_id"],'test_id'=>$map['exam_id']))->getField('freq');
			$where1 = array('user_id'=>$v['user_id'],'exam_id'=>$map['exam_id'],'project_id'=>$v['project_id']);
			$score = M('exam_score')->where($where1)->order($order)->field('total_score,counter')->select();
			if($score){
				foreach($score as $kk=>$vv){
					...
					$v['freq'] = $freq;
					$time = M('exam_answer')
						->where(array('exam_id'=>$map['exam_id'],'project_id'=>$map['project_id'],'u_exam_id'=>$v['user_id'],'counter'=>$vv['counter']))
...
//      $show=$this->pageClass($count,$total_page);
     ...
     * @return [type]      [description]
     */
    public function getTestScore($pid,$user_id,$eid,$max){

        $where['project_id']  = array("eq",$pid);
        $where['user_id']  = array("eq",$user_id);
        $where['counter']  = array("eq",$max);
        $eid = $eid ? $eid : 0;
	...
	*/
	...
	public function export(){
		...
		$data = $this->getAllExaminationResults2($total_page,$get);
		$info = $data['list'];
		foreach($info as $k=>$v){
			foreach($v as $kk=>$vv){
				if(!is_array($vv)){
		...
		foreach($temp as $k=>$v){
			$temp2[$k]['username'] = $v['username'];
			$temp2[$k]['name'] = $v['name'];
			$temp2[$k]['fname'] = $v['fname'];
			$temp2[$k]['attendance'] = $v['attendance'] == 1 ? '已参与' : '未参与';
			$temp2[$k]['num'] = $v['counter'] . ' / ' . $v['freq'];
			$temp2[$k]['total_score'] = $v['total_score'];
			$temp2[$k]['data_tiem'] = $v['data_tiem'];
		}
//		dump($temp2);die();
		return $temp2;
	}
