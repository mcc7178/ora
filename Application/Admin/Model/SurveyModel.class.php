    ...
    public function getProjectSurvey($total_page = 10){
       ...
       if(!empty($keyword)){
		...
		if(strtolower(C('DB_TYPE')) == 'oracle'){
			$field = "to_char(a.start_time,'YYYY-MM-DD HH24:MI:SS') as start_time,to_char(a.end_time,'YYYY-MM-DD HH24:MI:SS') as end_time,a.survey_names,a.survey_id,a.project_id,b.project_name,c.survey_name,d.cat_name";
		}else{
        		...
        		if($value["item_type"] == 1){
        			...
        			if($total > 0){
        				foreach ($itemOpt as $subKey=>$subValue){
							...
							if(strtolower(C('DB_TYPE')) == 'oracle'){
								$optNum = $answer->field("id,survey_answer")->where($optWhere)->select();
