...
class MySurveyModel extends BaseModel{
    ...
    public function getWaitSurvey($total_page = 10){
        ...
        if(strtolower(C('DB_TYPE')) == 'oracle'){
            ...
            $where = "a.user_id =". $_SESSION['user']['id']." and a.status != 2";
            ...
            if(!empty($keyword)){
                ...
                $count = M("survey_attendance a")->join("LEFT JOIN __ADMIN_PROJECT__ b ON a.project_id = b.id JOIN __PROJECT_SURVEY__ c ON b.id = c.project_id and c.survey_id = a.survey_id LEFT JOIN __SURVEY__ d ON c.survey_id = d.id LEFT JOIN __USERS__  f ON f.id = b.user_id")->where($where)->count();
            }
        }else{
    ...
    public function getSurveyInfo(){
		...
		$surveyItem = M("survey_item")->where("survey_id=".$survey_id)->order("orders asc")->select();
        ...
        try {
            ...
            if(strtolower(C('DB_TYPE')) == 'oracle'){
                ...
                if($data['typeid'] == 0){
                    $data_items = array(
                        'status'=>1,
                        'commit_time'=>array('exp',"to_date('".date('Y-m-d H:i:s')."','yy-mm-dd hh24:mi:ss')")
                    );
            ...
            if($data['typeid'] == 0){
                ...
                M('survey_attendance')->where($conditions)->setField($data_items);
                ...
                M('research_attendance')->where($conditions)->setField($data_items);
            }
    ...
    public function is_survey($k,$item,$typeid,$info,$type)
    {
        ...
        if($type == 0){
            ...
            if(strtolower(C('DB_TYPE')) == 'oracle'){
                $itea['id'] = getNextId('survey_answer');
            }
            ...
            if(strtolower(C('DB_TYPE')) == 'oracle'){
                $itea['id'] = getNextId('research_answer');
            }
    ...
    public function researchList($total_page){
        ...
        if(strtolower(C('DB_TYPE')) == 'oracle'){
            ...
            $where = "a.user_id =". $_SESSION['user']['id']." and a.state in(0,1,3) and b.audit_state != 0";
            ...
            $count = M("research_attendance a")->join("LEFT JOIN __RESEARCH__ b ON a.survey_id = b.survey_id and a.research_id = b.id LEFT JOIN __SURVEY__ c ON a.survey_id = c.id LEFT JOIN __SURVEY_CATEGORY__ d ON c.survey_cat_id = d.id LEFT JOIN __USERS__  f ON f.id = b.auth_user_id")->where($where)->count();
        }
     ...
     * 全部调研
     */
    public function surveyall($total_page){
        ...
        if(strtolower(C('DB_TYPE')) == 'oracle'){
            ...
            $survey_results = M("research_attendance a")
            	->join("LEFT JOIN __RESEARCH__ b ON a.survey_id = b.survey_id and a.research_id = b.id LEFT JOIN __SURVEY__ c ON a.survey_id = c.id LEFT JOIN __SURVEY_CATEGORY__ d ON c.survey_cat_id = d.id LEFT JOIN __USERS__ f ON f.id = b.auth_user_id")
            	->field("b.auth_user_id,a.state as status,b.research_name,b.id,b.survey_id,to_char(b.start_time,'YYYY-MM-DD HH24:MI:SS') start_time,to_char(b.end_time,'YYYY-MM-DD HH24:MI:SS') end_time,d.cat_name,f.username")
            	->order("a.state asc,b.start_time asc,b.end_time asc")
            	->where($where)
            	->select();
            ...
            foreach($survey_results as $k=>$results){
                if($results['auth_user_id'] == $_SESSION['user']['id']){
        ...
        $items = array_slice($results,$p,$total_page);
