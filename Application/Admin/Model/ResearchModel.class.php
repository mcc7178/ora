...
class ResearchModel extends BaseModel{
	...
	public function index($total_page){
		...
		if(strtolower(C('DB_TYPE')) == 'oracle'){
			...
			$results = M("research")
				->where($where)
				->field("id,research_name,survey_id,objection,to_char(start_time,'YYYY-MM-DD HH24:MI:SS') as start_time,to_char(end_time,'YYYY-MM-DD HH24:MI:SS') as end_time,auth_user_id")
	...
	public function editor(){
		...
		if(empty($post['id'])){
			...
			try {
				...
				if(strtolower(C('DB_TYPE')) == 'oracle'){
					$data['id'] = getNextId('research');
					$data['create_time'] = array('exp',"to_date('".date('Y-m-d H:i:s')."','yy-mm-dd hh24:mi:ss')");
					$data['start_time'] = array('exp',"to_date('".$post['start_time']."','yy-mm-dd hh24:mi:ss')");
					$data['end_time'] = array('exp',"to_date('".$post['end_time']."','yy-mm-dd hh24:mi:ss')");
				...
				if($research_id){
					...
					if (!$DB->autoCheckToken($post)){
						...
						write_login_log(9,2,$data['research_name']);
			...
			try {
				...
				write_login_log(8,3,$data['research_name']);
...
//			$str_tissue_name = implode(",",$tissue_name);
	...
	public function delete(){
		...
		$research_list = M("research")->field("research_name")->where($data)->select();

		foreach($research_list as $list){
			write_login_log(9,4,$list['research_name']);
		}
