...
class TestManageModel extends BaseModel{
     ...
     */
    public function getTestList($total_page=10){
		...
		if(strtolower(C('DB_TYPE')) == 'oracle'){
			$res = M('test')
				->where($where)
				->field("id,auth_user_id,name,create_user,type,status,audit_status,score,examination_id,address,pass_line,to_char(start_time,'YYYY-MM-DD HH24:MI:SS') as start_time,to_char(end_time,'YYYY-MM-DD HH24:MI:SS') as end_time")
    ...
    public function getAllExmination(){
        ...
        //分享给我的试卷
        $sharing = D('ResourcesManage')->sharingtome();
        $sharing = $sharing['list'] ? $sharing['list'] : array();

        $typeid = I('get.typeid',0,'int');
        $res = $typeid ? $sharing : $res;
    ...
    public function getAllExminationCate(){
        ...
        return M('examination_category')->order('sort desc')->where($where)->select();
    }
    ...
    public function addTest($data){
		...
		$data['freq'] = $data['freq'];
		$data['pass_line'] = $data['pass_score'];
        
        $orderInfo = D('Trigger')->orderNumber(7);
		$data['orderno'] = $orderInfo['no'];
        ...
        if($res){
			...
			//写入日志
			write_login_log(10,2,$data['name']);
            return true;
        }else{
            return false;
        }
    ...
    public function getTestInfo($id){
		...
		if(strtolower(C('DB_TYPE')) == 'oracle'){
			$field = "id,name,create_user,type,status,audit_status,score,examination_id,address,orderno,objection,pass_line,test_length,auth_user_id,to_char(start_time,'YYYY-MM-DD HH24:MI:SS') as start_time,to_char(end_time,'YYYY-MM-DD HH24:MI:SS') as end_time";
		}else{
			$field = "*";
		}
        $res = M('test')
     ...
     * @return [type] [description]
	 * @param $id 考试id
	 * @param $type 导出考试成绩的方式，0-只导出最高，1-导出全部
     */
    public function getTestuser_ids($id,$type=1){
        ...
        $res = M('test_user_rel')
                ...
                ->field('b.*,a.name as test_name,a.freq,c.username,c.phone,e.name as tissue_name,f.name as job_name,g.status as attendance_status')
                ->select();
		if($type == 0){
        ...
        foreach($res as $k=>$v){
        	$score = M('exam_score')
        		->where(array('test_id'=>$v['test_id'],'user_id'=>$v['user_id']))
        		->order($order)
        		->field('total_score,counter')
				->select();
			if($score){
				foreach($score as $kk=>$vv){
					...
					$v['total_score'] = $vv['total_score'];
					$v['counter'] = $vv['counter'];
					$time = M('exam_answer')
						->where(array('test_id'=>$v['test_id'],'u_exam_id'=>$v['user_id'],'counter'=>$vv['counter']))
						->field("to_char(data_tiem,'YYYY-MM-DD HH24:MI:SS') as data_tiem")
						->find();
					$v['data_tiem'] = $time['data_tiem'];
					$temp[] = $v;
				}
			}else{
				$v['total_score'] = 0;
    ...
    public function publish($data){
    	$info = M('exam_score')->where(array('test_id'=>$data['test_id']))->find();
		if($info){
     ...
     * @return [type] [description]
     */
    public function getUserInfo($tid,$user_id,$max=false){
     ...
     * @return [type]      [description]
     */
    public function getanswerInfo($max,$tid,$user_id,$eid=null,$pid=null){
        ...
        foreach($res as $k=>$v){
            $info = M('exam_answer')
                    ->where(array('exam_id'=>$eid,'u_exam_id'=>$user_id,'test_id'=>$tid,'question_number'=>$v['examination_item_id'],'counter'=>$max))
     ...
     * @return [type]      [description]
     */
    public function getWdScore($max,$tid,$user_id){
        return M('exam_answer')
                ->where(array('u_exam_id'=>$user_id,'test_id'=>$tid,'classification'=>4,'counter'=>$max))
                ->select();
    }
    ...
    public function getExamDetail($id){
		...
		if(strtolower(C('DB_TYPE')) == 'oracle'){
			$field = "a.id,a.name,a.create_user,a.type,a.status,a.audit_status,a.score,a.examination_id,a.address,a.orderno,a.objection,a.pass_line,a.test_length,a.auth_user_id,to_char(a.start_time,'YYYY-MM-DD HH24:MI:SS') as start_time,to_char(a.end_time,'YYYY-MM-DD HH24:MI:SS') as end_time";
		}else{
			$field = "a.*";
		}
        $ret = M('test a')
                ...
                ->where($where)
                ->field($field.",b.test_name,b.test_cat_id,b.test_score,b.test_heir,b.test_upload_time,b.is_available,b.principal,b.test_mode,c.cat_name")
                ->find();
    ...
    public function all(){
		...
		$type = I('get.type');
        $data = $this->getTestuser_ids($test_id,$type);
        $name = $data[0]['test_name'];
		$temp = array();
        foreach($data as $k=>$v){
        	$temp[$k]['name'] = $v['username'];
        	$temp[$k]['tissue_name'] = $v['tissue_name'] == '' ? '未分配' : $v['tissue_name'];
			$temp[$k]['job_name'] = $v['job_name'] == '' ? '未分配' : $v['job_name'];
			$temp[$k]['attendance_status'] = $v['attendance_status'] == 0 ? '未参与' : '已参与';
        	$temp[$k]['total_score'] = !$v['total_score'] ? '0' : $v['total_score'];
        	$temp[$k]['test_num'] = $v['counter'] . ' / ' . $v['freq'];
        	$temp[$k]['data_tiem'] = !$v['data_tiem'] ? '---' : $v['data_tiem'];
        }
        return array('name'=>$name,'data'=>$temp);
    }
