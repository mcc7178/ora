...
class IntegrationModel extends BaseModel{
    ...
    public function shareTome(){
        ...
        if(!empty($keyword)){
            $conditions['b.name'] = array("like","%$keyword%");
        }
        //type为9表示福利类型
        $conditions['a.type'] = 9;
        $conditions['d.user_id'] = session('user.id');

        $field = "b.id,b.name,b.need_score,b.auth_user_id,b.welfare_covers,b.welfare_describe,b.add_time,b.update_time";
        $data = M('resource_sharing')
            ->alias('a')
            ->join('left join __WELFARE__ b on a.source_id=b.id')
     ...
     */
     public function integrationlist()
	 {
         ...
         if($list1){//如果是查询到该用户所在的组织有应用到方案的，则执行这里
            ...
            // $result = M('integration_rule_default')->select();
             foreach($_list as $k => $v){
                 if(!empty($plan_id)){
                     $v['plan_id'] = $plan_id;
                     // $result[$k]['plan_id'] = $v['plan_id'];
     ...
     */
     public function editIntegrationList()
	 {
         ...
         if(!empty($planId)){
             ...
             $res = M('integration_rule')->where(array('id'=>$id, 'plan_id' => $planId))->save($map);
         }
          return $res;
     }
     ...
     */
     public function integrationHistoryList()
	 {
      ...
      if(strtolower(C('DB_TYPE')) == 'oracle'){
       $list2 = M('integration_record')->alias('a')
                                      ->field("a.id,a.time,a.user_id,a.department,a.score,a.type,a.describe,a.apply_id,b.username,d.id as did ")
                                      ->join('left join __USERS__ b on b.id = a.user_id')
                                      ->join('left join __TISSUE_GROUP_ACCESS__ c on c.user_id = a.user_id')
                                      ->join('left join __TISSUE_RULE__ d on d.id = c.tissue_id')
                                      ->where($where)
     ...
     public function integralExchange(){
         ...
         if($exchange){//如果有数据则获取
             $_exchange = $exchange;
         }else{//如果没有则默认的数据
             $_exchange = M("integration_exchange")->where(array('plan_id' => 0))->find();
             $data["exc_rule"] = 0;
             $data["update_time"] = date("Y-m-d H:i:s");
             $data["auth_user_id"] = intval(session('user.id'));
             $data["plan_id"] = intval($plan_id);
             M("integration_exchange")->add($data);
         }
     	return $_exchange;
     }
     ...
     * @return array
     */
     public function saveExchangeRule($ruleVal,$planId){

     	 $data["exc_rule"] = $ruleVal;
     	 $data["update_time"] = date("Y-m-d H:i:s");
     	 $data["auth_user_id"] = intval(session('user.id'));
         if(!empty($planId)){
             $result = M("integration_exchange")->where(array('plan_id' => $planId))->save($data);
         }
         return $result;
     }
    ...
    **/ 
     ...
     * 我的积分列表展示
     */
     public function myintegration($tabType)
	 {
      //我的积分模块的列表页显示：总积分，可用积分，本月积分
        ...
        if(IS_AJAX){
         ...
         $month = I('post.months')=='' ? date("m") : I('post.months');;
         $months = $year.$month;
