...
class AuditModel extends Model{
     ...
     * 审核初始数据
     */
     public function auditDataInit($planId)
     {

       $res = M('audit_rule')->where(array('plan_id'=>$planId))->find();
       
       if(!$res){
         
         //初始审核数据
         $rule_data = array(
           '1'=>array('type'=>1,'is_condition'=>2,'plan_id'=>$planId),
           '2'=>array('type'=>2,'is_condition'=>2,'plan_id'=>$planId),
           '3'=>array('type'=>3,'is_condition'=>2,'plan_id'=>$planId),
           '4'=>array('type'=>4,'is_condition'=>2,'plan_id'=>$planId),
           '5'=>array('type'=>5,'is_condition'=>2,'plan_id'=>$planId),
           '6'=>array('type'=>6,'is_condition'=>2,'plan_id'=>$planId),
           '7'=>array('type'=>7,'is_condition'=>2,'plan_id'=>$planId),
           '8'=>array('type'=>8,'is_condition'=>2,'plan_id'=>$planId),
           '9'=>array('type'=>9,'is_condition'=>2,'plan_id'=>$planId),
           '10'=>array('type'=>10,'is_condition'=>2,'plan_id'=>$planId),
          //  '11'=>array('type'=>11,'is_condition'=>2,'plan_id'=>$planId),
           '12'=>array('type'=>12,'is_condition'=>2,'plan_id'=>$planId),
         );
         
         $condition_data = array(
           '1'=>array('name'=>'项目时长（天）','conditiona'=>0,'conditionb'=>0,'type'=>1,'plan_id'=>$planId),
           '2'=>array('name'=>'项目预算（元）','conditiona'=>0,'conditionb'=>0,'type'=>1,'plan_id'=>$planId),
           '3'=>array('name'=>'指定人员（人数））','conditiona'=>0,'conditionb'=>0,'type'=>1,'plan_id'=>$planId),
           '4'=>array('name'=>'授课时长（分钟）','conditiona'=>0,'conditionb'=>0,'type'=>2,'plan_id'=>$planId),
           '5'=>array('name'=>'新建课程学分（分）','conditiona'=>0,'conditionb'=>0,'type'=>2,'plan_id'=>$planId),
           '6'=>array('name'=>'持续时长（天）','conditiona'=>0,'conditionb'=>0,'type'=>6,'plan_id'=>$planId),
          //  '7'=>array('name'=>'学分（分）','conditiona'=>0,'conditionb'=>0,'type'=>66,'plan_id'=>$planId),
           '8'=>array('name'=>'考试时长（分钟）','conditiona'=>0,'conditionb'=>0,'type'=>7,'plan_id'=>$planId),
          //  '9'=>array('name'=>'学分（分）','conditiona'=>0,'conditionb'=>0,'type'=>77,'plan_id'=>$planId),
           '10'=>array('name'=>'指定人员（人数）','conditiona'=>0,'conditionb'=>0,'type'=>7,'plan_id'=>$planId),
           '11'=>array('name'=>'学分（分）','conditiona'=>0,'conditionb'=>0,'type'=>8,'plan_id'=>$planId),
           '12'=>array('name'=>'人数上限','conditiona'=>0,'conditionb'=>0,'type'=>12,'plan_id'=>$planId),
         ); 
        ...
        try {
         ...
         foreach($rule_data as $k=>$v){
           ...
           if(strtolower(C('DB_TYPE')) == 'oracle'){
              $v['id'] = getNextId('audit_rule');
            }
           $ret = M('audit_rule')->add($v);
         }
       

         foreach($condition_data as $k1=>$v1){
           if(strtolower(C('DB_TYPE')) == 'oracle'){
              $v1['id'] = getNextId('audit_condition');
            }
           $ret = M('audit_condition')->add($v1);
         }
     ...
     */
     ...
     { 
       $planId = getPlanId();
       $res = M('audit_condition')->where(array('type'=>$type,'plan_id'=>$planId))->select();
       return $res;
     }
     ...
     */
     ...
     { 
       $planId = getPlanId();
       $data = M('audit_rule')->where(array('plan_id'=>$planId))->select();
       ...
       foreach($data as $k=>$v){
        ...
        //    $data[$k]['name'] = $setdata[$key];
           $data[$k]['name'] = $setdata[$v['type']];
           //审核类型名
           if($v['is_condition'] == 0){  //审核类型设置：0无条件审批流程，1条件审批流程，2无需审核
             $data[$k]['typename'] = '无条件审批流程';
           }if($v['is_condition'] == 1){ 
             $data[$k]['typename'] = '条件审批流程';
           }if($v['is_condition'] == 2){ 
             $data[$k]['typename'] = '无需审核';
             $data[$k]['num'] = '--';
           }
   ...
   public function uname($user_id){
     ...
     */
     ...
     { 
        ...
        $data = M('audit_rule')->alias('a')
                       ...
                       ->where(array('a.type'=>$type,'a.plan_id'=>$planId))
     ...
     */
     public function saveSet()
     {
      ...
      //  dump($data);exit;  
       ...
       //1:培训项目,2:新建课程,3:新建试卷审,4:新建问卷,5:新建互动,6:发起调研7:发起考试,8:发起加分,9:用户注册,10:话题小组,11:申请加学分12:活动报名
       $typename = array('培训项目','新建课程','新建试卷审','新建问卷','新建互动','发起调研','发起考试','发起加分','用户注册','话题小组','申请加学分','活动报名');
       $msg = $typename[$data['type']-1];
       ...
       if($data['is_condition'] == 0){ //无条件
         ...
         $exist = M('audit_rule')->where(array('type'=>$data['type'],'plan_id'=>$planId))->find();

         if($exist){
           
           $res = M('audit_rule')->where(array('type'=>$data['type'],'plan_id'=>$planId))->save($data);
            ...
            if($type == 1){ //项目审核-拒绝
             foreach($ids as $k=>$v){
               ...
               $ret = array(
                     'status'=>1,
                     'info'=>'批量审核拒绝成功',
					'url'=>U('Admin/Audit/activityauditlist')
                      );
                return $ret; 
            }
               ...
               if($dataone['is_condition'] == 0){ //无条件
                ...
                $res = M('admin_project')->where(array('id'=>$dataone['correlate_id']))->save(array('type'=>0));
                 ...
                 $designeeNum = M('admin_project')->alias('a')
                              ...
                              ->where(array('a.id'=>$dataone['correlate_id'],'typeid'=>0))
                              ->count();
              ...
              return $ret;
               ...
               if($dataone['is_condition'] == 0){ //无条件
                ...
                $res = M('research')->where(array('id'=>$dataone['correlate_id']))->save(array('audit_state'=>1));
                  ...
                  if(strtolower(C('DB_TYPE')) == 'oracle'){
                    $tianjian = M('research')
                            ->field("id,credits,to_char(start_time,'YYYY-MM-DD HH24:MI:SS') as start_time,to_char(end_time,'YYYY-MM-DD HH24:MI:SS') as end_time")
                            ->where(array('id'=>$dataone['correlate_id']))
                            ->find();
                  }else{
             ...
             return $ret;
               ...
               if($dataone['is_condition'] == 0){ //无条件
                ...
                $res = M('test')->where(array('id'=>$dataone['correlate_id']))->save(array('audit_status'=>0));
                  ...
                  $test_time = $tianjian['test_length'];
       ...
       if($type == 1){
           ...
           if(strtolower(C('DB_TYPE')) == 'oracle'){
               ...
               $lists = M('friends_circle')->alias('a')
                   ...
                   ->page($p1.','.$size)
                   ->order('a.id desc')
                   ->field('a.id,a.content,a.orderno,a.user_id,b.oneaudit_user_id,b.twoaudit_user_id,b.threeaudit_user_id,b.one_level_type,b.two_level_type,b.three_level_type,b.id as audit_id,b.levalone_man,b.levaltwo_man,b.levalthree_man,b.audit_status,b.objection,b.oneaudit_role,b.twoaudit_role,b.threeaudit_role,b.is_condition,b.condition_id,b.conditiona,b.conditionb,b.num')
                   ->select();
           ...
           if(strtolower(C('DB_TYPE')) == 'oracle'){
               ...
               $lists = M('users')->alias('a')
                   ->join('left join __AUDIT__ b on b.correlate_id = a.id')
                   ->field("a.id,a.phone,a.username,a.orderno,b.oneaudit_user_id,b.twoaudit_user_id,b.threeaudit_user_id,b.one_level_type,b.two_level_type,b.three_level_type,b.id as audit_id,b.levalone_man,b.levaltwo_man,b.levalthree_man,b.audit_status,b.objection,b.oneaudit_role,b.twoaudit_role,b.threeaudit_role,b.is_condition,b.condition_id,b.conditiona,b.conditionb,b.num")
     ...
     */
     ...
     {  
      $population = M('designated_personnel')->field('count(*) as population')->where(array('project_id'=>$id,'typeid'=>0))->select();
     ...
     */
     public function threeallauditdetail()
     {
        ...
        if($tablename == 'admin_project'){
              ...
              if(strtolower(C('DB_TYPE')) == 'oracle'){
              $res1 = $audit->alias('a')
                            ->field("a.id as aid,a.tissue_id,a.project_name,a.type,a.project_covers,a.class_name,to_char(a.start_time,'YYYY-MM-DD HH24:MI:SS') as start_time,to_char(a.end_time,'YYYY-MM-DD HH24:MI:SS') as end_time,a.project_description,a.project_budget,b.username")
                            ->join(' LEFT JOIN __USERS__ b  on b.id = a.user_id')
                            ->where("a.id = $get")
                            ->find();
              //项目指定人员
              $persons =  $audit->alias('a')
                            ->field('c.username')
                            ->join('LEFT JOIN __DESIGNATED_PERSONNEL__ b on a.id = b.project_id')
                            ->join('LEFT JOIN __USERS__ c on b.user_id = c.id')
                            ->where(array('a.id'=>$get,'b.typeid'=>0))
                            ->select();
               $res1['uname'] = $persons;         

              //项目下的课程详情
                $res2 = $audit->field('b.*')->alias('a')
                            ->field("c.course_name,c.course_way,c.lecturer,c.lecturer_name,d.name,to_char(b.start_time,'YYYY-MM-DD HH24:MI:SS') as start_time,to_char(b.end_time,'YYYY-MM-DD HH24:MI:SS') as end_time,b.credit,b.location")
            ...
            //项目下的考试详情
                $res2 = $audit->alias('a')
                          ->field("b.cacheid,b.test_names,c.test_mode,d.username,to_char(b.start_time,'YYYY-MM-DD HH24:MI:SS') as start_time,to_char(b.end_time,'YYYY-MM-DD HH24:MI:SS') as end_time,b.credits,b.test_length as test_length")
              ...
              //项目下的调研详情
              $res2 = $audit->alias('a')
                          ->field("c.survey_name,d.username,to_char(b.start_time,'YYYY-MM-DD HH24:MI:SS') as start_time,to_char(b.end_time,'YYYY-MM-DD HH24:MI:SS') as end_time,b.credit")
               ...
               foreach($res['tag_ids'] as $k=>$v){
                ...
                $res = $coursem->field('a.id,a.status,a.survey_name,a.survey_score,c.username as survey_heir,a.survey_upload_time,b.cat_name')
                  ...
                  $surveyitem = $question
                                ...
                                ->where($where)
                                ->order('a.classification asc,a.id asc')
                                ->select();
                ...
                if(strtolower(C('DB_TYPE')) == 'oracle'){ 
                    ...
                    $data = M('friends_circle')->alias('a')
                        ->field("a.user_id,a.status,to_char(a.publish_time,'YYYY-MM-DD HH24:MI:SS') as publish_time,a.content,b.username,'话题审核' auditTitle")
                        ->join('left join __USERS__ b on b.id=a.user_id')
                        ->where(array('a.id'=>$id))
                        ->find(); 
                ...
                if(strtolower(C('DB_TYPE')) == 'oracle'){ 
                    $title = M('research')
                      ...
                      ->where(array('a.id'=>$id))
                      ->field("a.id,a.research_name,a.survey_id,a.credits,b.username,to_char(a.create_time,'YYYY-MM-DD HH24:MI:SS') as create_time,to_char(a.start_time,'YYYY-MM-DD HH24:MI:SS') as start_time,to_char(a.end_time,'YYYY-MM-DD HH24:MI:SS') as end_time,a.audit_state,d.name")
                      ->select(); 
                }else{
                    $title = M('research')
                      ->alias('a')
                ...
                if($research_data){
                  ...
                  $surveyItem = M("survey_item")->where("survey_id=".$research_data["survey_id"])->order("orders asc")->select();
          ...
          if(strtolower(C('DB_TYPE')) == 'oracle'){ 
              $title = M('test')
                      ...
                      ->where(array('a.id'=>$id))
                      ->field("a.id,a.name,a.examination_id,a.type,a.score,c.test_score,b.username,to_char(a.create_time,'YYYY-MM-DD HH24:MI:SS') as create_time,to_char(a.start_time,'YYYY-MM-DD HH24:MI:SS') as start_time,to_char(a.end_time,'YYYY-MM-DD HH24:MI:SS') as end_time,a.audit_status,a.address")
                      ->find();
          }else{
              $title = M('test')
                      ->alias('a')
          ...
          //  print_r($designee); 
            ...
            return $data;
             ...
             return $data;
              ...
              if(strtolower(C('DB_TYPE')) == 'oracle'){ 
                   $data = M('users')->field("username,to_char(register_time,'YYYY-MM-DD HH24:MI:SS') as register_time,status,email,phone,job_number,province,city")->where(array('id'=>$id))->find(); 
             ...
             if(strtolower(C('DB_TYPE')) == 'oracle'){ 
              $data = M('topic_group')->alias('a')
                       ->join('left join __USERS__  b on b.id=a.user_id')
                       ->where(array('a.id'=>$id))
                       ->field("a.id,a.user_id,a.name,a.theme,a.status,a.orderno,a.group_covers,to_char(a.publish_time,'YYYY-MM-DD HH24:MI:SS') as publish_time,b.username")
                       ->find();
              }else{
               $data = M('topic_group')->alias('a')
                       ->join('left join __USERS__  b on b.id=a.user_id')
            ...
            if(strtolower(C('DB_TYPE')) == 'oracle'){
             $data = M('activity')->alias('a')
                    ->field("a.id,a.type,a.user_id,a.activity_name,a.address,a.activity_covers,a.activity_description,to_char(a.add_time,'YYYY-MM-DD HH24:MI:SS') as add_time,to_char(a.activity_start_time,'YYYY-MM-DD HH24:MI:SS') as activity_start_time,to_char(a.activity_end_time,'YYYY-MM-DD HH24:MI:SS') as activity_end_time,to_char(a.apply_start_time,'YYYY-MM-DD HH24:MI:SS') as apply_start_time,to_char(a.apply_end_time,'YYYY-MM-DD HH24:MI:SS') as apply_end_time,b.username")
                    ->join('left join __USERS__ b on b.id=a.user_id')
                    ->where(array('a.id'=>$id))
                    ->find();
             ...
             $data = M('activity')->alias('a')->field('a.*,b.username')
                    ->join('left join __USERS__ b on b.id=a.user_id')
                    ->where(array('a.id'=>$id))
                    ->find();  
           }
