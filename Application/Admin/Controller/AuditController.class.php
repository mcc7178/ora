...
class AuditController extends AdminBaseController{
     ...
     */
     public function setList()
     {
       ...
       //检测登录者所在组织是否设置配置方案，没有则提示“您所在的组织请先设置配置方案”
        $planId = getPlanId();
        if(!$planId){
          echo "您所在的组织请先设置配置方案";
          return false;
       ...
       //检测审核规则数据表是否已存在相应配置方案的数据，不存在则添加初始数据（初始审核数据默认无需审核）
        $res = D('Audit')->auditDataInit($planId);
     ...
     *活动报名审核列表
     */
     public function activityauditlist()
     {  

        $data = D('Audit')->threeallauditlist(12);
        $tabType = I('get.tabType') !== '' ? I('get.tabType') : '1';
        if($tabType == 1){
         $this->assign('list1',$data['lists']);
         $this->assign('page1',$data['page']);
