...
class CenterController extends AdminBaseController{
	...
	public function study(){
		...
		if($get["type"] != 1 && $get["type"] != 2 && $get["type"] != 3 && $get["type"] != 4){
			$get["type"] = 1;
		}
		...
		$data["type"] = $get["type"];
		$data["startTime"] = $get["startTime"];
		$data["endTime"] = $get["endTime"];
		$data["circleType"] = (int)$get["circleType"];
		
		$data["iacStatus"] = D("Iac")->getIacStatus();//是否开启中保协
	...
	public function exchangePage(){
		...
		$data = D("Center")->exchangePage($page);
		$this->assign($data);
		$this->display("exchange_page");
	}
	...
	public function exchange(){
		$post = I("post.");
		$post["excVal"] = (int)$post["excVal"];
		$post["excRule"] = (int)$post["excRule"];
		if($post["excVal"] < 1){
			exit(json_encode(array("code"=>1001, "message"=>"输入学分必须为大于0的整数")));
		}
		
		if($post["excRule"] < 1){
			exit(json_encode(array("code"=>1002, "message"=>"当前兑换率必须为大于0的整数")));
		}
		
		$valid_credits = F("valid_credits");
		if($post["excVal"] > $valid_credits){
			exit(json_encode(array("code"=>1003, "message"=>"可兑换学分不足")));
		}
		
		$data = D("Center")->exchange($post);
	...
	public function studyExport(){
		...
		if($get["type"] == 1){
			...
			$fileName = urlencode("中保协").date("YmdHi");//需要编码，否则IE下文件名称错误
			$title[] = "序号";
			$title[] = "课程名称";
			$title[] = "学习时长(秒)";
			$title[] = "获得学分";
		...
		while(true){
			...
			foreach($getDownload as $thisValue){
				...
				if($get["type"] == 1){
					...
					$newRow[] = $dataOrder;
					$newRow[] = $row["course_name"];
					$newRow[] = $row["study_len"];
					$newRow[] = $row["get_credit"];
				}
