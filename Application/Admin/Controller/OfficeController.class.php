...
class OfficeController extends AdminBaseController{
	...
	public function index(){
		//使用示例
		$url = 'http://ys3.occupationedu.com/Upload/resources/20170926/59ca2eb6c1216.docx';
		$data = D("RsCourse")->fileConvert($url);
		print_r($data);
	}
	...
	public function convert($inputFile){
		...
		$judgeOffice = shell_exec("soffice --version");
		if(strstr($judgeOffice, "command not found")){
			return array("code"=>1002, "message"=>"不支持office转换，请确保已正确安装libreoffice");
		}
		...
		if($linuxReturn){
			...
			rmdir($outPath);
			$return = array("code"=>1004, "message"=>"system error,系统错误,请确保路径目录权限为777", "cmd"=>$cmd);
		}
		return $return;
	}
