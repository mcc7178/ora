...
class ManageController extends AdminBaseController
{
    ...
    public function submit(){
        ...
        //如果存在编辑ID,写入缓存
        if(!empty($post['project_id'])){
            //如果返回不读取数据库数据,直接读取缓存数据
            if(empty($post['edit_cache'])){
	...
	public function offlineTraining(){
		$data = D('Manage')->getTrainingData();
		
		$this->assign('list',$data['list']);
		$this->assign('typeid',$data['typeid']);
		$this->assign('page',$data['page']);
		$this->assign('keyword',$data['keyword']);
		$this->assign('plan_category',$data['plan_category']);
		$this->assign('training_category',$data['training_category']);
		$this->display();
	}
	...
	public function detail2(){
		$data = D('Manage')->detail2();
		
		$this->assign('base',$data['base']);
