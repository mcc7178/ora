    ...
    */
     ...
     */
     public function add()
     {
        ...
        if($res){
          $this->success($info,U('Admin/TopicGroup/index'));
        }else{
          $this->error(D('TopicGroup')->getError());
        }
        ...
        //  echo $res;
         if($res){
            if(strtolower(C('DB_TYPE')) == 'oracle'){
             $arr = M('topic_interaction')->where(array('id'=>$res))->field("to_char(publish_time,'YYYY-MM-DD HH24:MI:SS') as publish_time")->find();
            }else{
             $arr = M('topic_interaction')->where(array('id'=>$res))->field("publish_time")->find();
            }
