...
class MyCourseController extends AdminBaseController{
    ...
    public function detail(){
    	...
    	//project_id值数字 true fasle,数字：由培训项目添加的课程，false公开课还未关注，ture公开课已关注，用于页面关注按钮 是否可评价的判断
		...
		//新的可查看考勤规则
        $data["whetherattendance"] = $get['whetherattendance'];
        if($get["whetherattendance"] == "1"){
	    	$data["kaoqin"] = "attendance/index/attendance_project_id/".$get["attendance_project_id"]."/project_id/".$get["project_id"]."/type/".$get["type"]."/flag/1/from_teach/1";
    	}
    ...
    public function detailTab(){
    	...
    	//echo "<br>ss001--".microtime();
    	$base = D("MyCourse")->getBaseInfo($get);
    	$data["base"] = $base["data"];
    	$data["course_bak"] = $base["course_bak"];
    	$data["course_id"] = $get["course_id"];
    	$data["project_id"] = $get["project_id"];
    ...
    public function chapter(){
    	...
    	if($post["fileType"] == "kejian" && strstr($post["fileSrc"], "scorm/course")){
    		$post["status"] = 2;//学习中
    		$post["timeLen"] = 0;
    		$post["time_percent"] = 0;
    ...
    public function setCare(){
    	...
    	if($id < 1){
    		exit(json_encode(array("code"=>1001, "message"=>"未获取到课程id")));
    	}
    	
    	$where["user_id"] = $user_id;
    	$where["course_id"] = $id;
    	$has = M("course_care")->where($where)->find();
    	if($has){
	    	$data["care_status"] = $post["status"];
	    	M("course_care")->where($where)->save($data);
    	}else{
    		$data["care_status"] = $post["status"];
    		$data["user_id"] = $user_id;
    		$data["course_id"] = $id;
    ...
    private function apiReq($apiName, $param){
    	...
    	$domain = $_SERVER['SERVER_NAME'];//获取当前服务器地址
    	$mainHost = $domain."/scorm/course/";
    	...
    	$param["scoUrl"] = str_replace("&amp;", "&", $param["scoUrl"]);
